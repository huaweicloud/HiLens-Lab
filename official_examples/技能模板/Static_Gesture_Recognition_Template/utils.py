...
# 可识别的手势类别，前4种为有效手势
class_names = ['OK', 'Rock', 'Yeah', 'Great', 'Other']
...
# 截取出OK手势图片，如果没有返回None
def get_OK(img_data, bboxes):
    for bbox in bboxes:
        label = int(bbox[4])
        if label == 0: # OK手势
            x_min = int(bbox[0])
            ...
            x_max = int(bbox[2])
            y_max = int(bbox[3])
            img_OK = img_data[y_min:y_max, x_min:x_max, :]
            return img_OK
    return None
...
def draw_boxes(img_data, bboxes):
    ...
    for bbox in bboxes:
        ...
        cv2.putText(img_data, text, (x_min, y_min+50), text_font, font_scale, colors[label], thickness)

    return img_data
    
# 以POST方式传输检测结果
def post_msg(server_url, img_rgb):
